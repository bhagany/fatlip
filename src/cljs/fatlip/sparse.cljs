(ns fatlip.sparse
  (:require [clojure.set :as set]
            [clojure.string :as s]
            [fatlip.protocols :refer [Sparse Layered Directed Nodey Reversible
                                      Node Edge rev map->Node]]))
...
(defn input->SparseGraph
  "Creates a sparse graph, as defined in ESK"
  [input]
  (let [graph (reduce
               (fn [g i]
                 (if-let [dup (->> (mapcat #(:characters %) (:groups i))
                                   frequencies
                                   (filter (fn [[_ c]] (> c 1)))
                                   seq)]
                   (throw (js/Error.
